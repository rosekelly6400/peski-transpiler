...
struct hash_table *
hash_table_create (int bucket_count, hash_func_t func)
{
  struct hash_table *h;
  ...
  h->buckets =
    (struct entry **) calloc (bucket_count, sizeof (struct entry *));
  if (!h->buckets)
    {
      free (h);
      return 0;
...
void
hash_table_clear (struct hash_table *h)
{
  struct entry *e, *f;
  int i;
...
void
hash_table_delete (struct hash_table *h)
{
  hash_table_clear (h);
  free (h->buckets);
  free (h);
...
void *
hash_table_lookup (struct hash_table *h, const char *key)
{
  struct entry *e;
  unsigned hash, index;
...
int
hash_table_size (struct hash_table *h)
{
  return h->size;
}

static int
hash_table_double_buckets (struct hash_table *h)
{
  struct hash_table *hn =
    hash_table_create (2 * h->bucket_count, h->hash_func);
...
int
hash_table_insert (struct hash_table *h, const char *key, const void *value)
{
  struct entry *e;
  unsigned hash, index;
...
void *
hash_table_remove (struct hash_table *h, const char *key)
{
  struct entry *e, *f;
  void *value;
  unsigned hash, index;
...
void
hash_table_firstkey (struct hash_table *h)
{
  h->ientry = 0;
...
int
hash_table_nextkey (struct hash_table *h, char **key, void **value)
{
  if (h->ientry)
    {
      *key = h->ientry->key;
     ...
     register ub4 initval;	/* the previous hash, or an arbitrary value */
{
  ...
  while (len >= 12)
    {
      a +=
	(k[0] + ((ub4) k[1] << 8) + ((ub4) k[2] << 16) + ((ub4) k[3] << 24));
      b +=
	(k[4] + ((ub4) k[5] << 8) + ((ub4) k[6] << 16) + ((ub4) k[7] << 24));
      c +=
	(k[8] + ((ub4) k[9] << 8) + ((ub4) k[10] << 16) +
	 ((ub4) k[11] << 24));
      mix (a, b, c);
      k += 12;
      len -= 12;
